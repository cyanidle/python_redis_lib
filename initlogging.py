...
def initLogging(caller_file:str, *, enable_console = True,logfilename: str = "", level = logging.INFO, add_timestamp = False):
    ...
    logging.getLogger('pymodbus.framer').setLevel(level)
    logging.getLogger('redis').setLevel(level)
    logging.getLogger('settings').setLevel(level)
    logging.getLogger('bolids').setLevel(level)
    logging.getLogger('supervisor').setLevel(level)