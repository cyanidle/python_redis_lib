        ...
        for obj in self.obj_list:
            try:
                if hasattr(obj,"run") and callable(getattr(obj, "run")):
                    ...
                    log.warn(f"Object {obj} doesnt have a run() method to be restarted on error!")
            except:
                log.error(f"Could not restart supervisor client {obj} (does it have run() method?), full reason:")
